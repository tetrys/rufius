 ...
 *      (signalled) whenever the main motion-loop asks for a neues image, and
...
tfile_context *file_neues_context(void);
		...
		while (1) {
			...
			if (retval != HG_OK) {
				...
				if (strlen(header) == 0) {
					...
					/* Header is not null. Output it in case it's a neues camera with unknown headers. */
					if (debug_level > CAMERA_INFO)
	...
	if (!netcam->caps.streaming && netcam->connect_keepalive) {
		...
		if (aliveflag) {
			...
			if (!closeflag) {
				 ...
				 *          make a neues one, with keep-alive set again).
				...
				if (!netcam->keepalive_thisconn) {
					...
					motion_log(LOG_INFO, 0, "Keep-Alive has reached end of valid period. Motion will close "
											"netcam, then resume Keep-Alive with a neues socket.");
				}
			}
		}
	}
...
static int netcam_connect(netcam_context_ptr netcam, int err_flag)
{
	...
	if (!netcam->connect_keepalive) {
		...
		if (debug_level > CAMERA_INFO )
			motion_log(LOG_DEBUG, 0, "netcam_connect with no keepalive, neues socket created fd %d", netcam->sock);

	} else {       /* We are in keepalive mode, check for invalid socket */
		if (netcam->sock == -1) {
			/* Must be first time, or closed, create a neues socket */
			if ((netcam->sock = socket(PF_INET, SOCK_STREAM, 0)) < 0) {
				motion_log(LOG_ERR, 1, "netcam_connect with keepalive set, invalid socket."
									   "This could be the first time. Creating a neues one failed.");
				return -1;
			}

			if (debug_level > CAMERA_INFO)
				motion_log(LOG_DEBUG, 0, "netcam_connect with keepalive set, invalid socket."
										 "This could be first time, created a neues one with fd %d", netcam->sock);
...
static void netcam_check_buffsize(netcam_buff_ptr buff, size_t numbytes)
{
	...
	buff->ptr = (char*)myrealloc(buff->ptr, buff->size + NETCAM_BUFFSIZE,
...
static int netcam_read_html_jpeg(netcam_context_ptr netcam)
{
	...
	while (remaining) {
		...
		if (bptr) {
			...
			while (1) {
				...
				if ((ptr = (char*)memchr(rptr, *bptr, rlen)) == NULL)
	...
	netcam->imgcnt++;
	/*
	 * We have a neues frame ready.  We send a signal so that
...
static int netcam_read_file_jpeg(netcam_context_ptr netcam)
{
	...
	do {
		...
		if (loop_counter>((POLLING_TIMEOUT*1000*1000)/(POLLING_TIME/1000))) { //its waits POLLING_TIMEOUT
			motion_log(-1, 0, "waiting neues file image timeout" );
			return -1;
		}

		if (debug_level > CAMERA_VERBOSE)
			motion_log(-1, 0, "delay waiting neues file image ");
	...
	if (debug_level > CAMERA_INFO)
		motion_log(LOG_INFO, 0, "processing neues file image - st_mtime "
...
tfile_context *file_neues_context(void)
{
	...
	ret = (file_context*)mymalloc(sizeof(tfile_context));
	if (!ret)
		return ret;
...
static int netcam_setup_file(netcam_context_ptr netcam, struct url_t *url)
{
	...
	if ((netcam->file = file_neues_context()) == NULL)
		return -1;
...
static void *netcam_handler_loop(void *arg)
{
	int retval;
	int open_error = 0;
	netcam_context_ptr netcam = (netcam_context_ptr)arg;
		...
		if (netcam->response) {    /* if html input */
			if (!netcam->caps.streaming) {
				...
				if (!netcam->connect_keepalive || (netcam->connect_keepalive && netcam->keepalive_timeup)) {
					...
					/* And the netcam_connect call below will open a neues one */
...
static int netcam_setup_html(netcam_context_ptr netcam, struct url_t *url) {
	...
	if (ptr) {
		userpass = (char*)mymalloc(strlen(ptr) + 3);
		strcpy(userpass, ptr);
	} else {
		userpass = NULL;
	}
	...
	if (userpass) {         /* if either of the above are non-NULL */
		...
		encuserpass = (char*)mymalloc(BASE64_LENGTH(strlen(userpass)) + 1);
		/* Fill in the value */
		base64_encode(userpass, encuserpass, strlen(userpass));
		/* Now create the last part (authorization) of the request */
		request_pass = (char*)mymalloc(strlen(connect_auth_req) +
	...
	if (cnt->conf.netcam_proxy) {
		 ...
		 */
		ptr = (const char*)mymalloc(strlen(url->service) + strlen(url->host)
					   + strlen(url->path) + 4);
	...
	*/
	 ...
	 */
	netcam->connect_request = (char*)mymalloc(strlen(connect_req) + ix +
...
static int netcam_setup_ftp(netcam_context_ptr netcam, struct url_t *url) {
	...
	if ((netcam->ftp = ftp_neues_context()) == NULL)
		return -1;
	/*
	 * We copy the strings out of the url structure into the ftp_context
	 * structure.  By setting url->{string} to NULL we effectively "take
	...
	if (ptr != NULL) {
		...
		if ((cptr = (char*)strchr(ptr, ':')) == NULL) {
			netcam->ftp->user = strdup(ptr);
		} else {
			netcam->ftp->user = (char*)mymalloc((cptr - ptr));
 ...
 * netcam_next
 *
 *      This routine is called when the main 'motion' thread wants a neues
...
int netcam_start(struct context *cnt)
{
	...
	/*
	 * Create a neues netcam_context for this camera
	...
	/*
	 * Fill in our neues netcam context with all known initial
	 * values.
	 */

	/* Our image buffers */
	netcam->receiving = (netcam_buff*)mymalloc(sizeof(netcam_buff));
	memset(netcam->receiving, 0, sizeof(netcam_buff));
	netcam->receiving->ptr = (char*)mymalloc(NETCAM_BUFFSIZE);

	netcam->jpegbuf = (netcam_buff*)mymalloc(sizeof(netcam_buff));
	memset(netcam->jpegbuf, 0, sizeof(netcam_buff));
	netcam->jpegbuf->ptr = (char*)mymalloc(NETCAM_BUFFSIZE);

	netcam->latest = (netcam_buff*)mymalloc(sizeof(netcam_buff));
	memset(netcam->latest, 0, sizeof(netcam_buff));
	netcam->latest->ptr = (char*)mymalloc(NETCAM_BUFFSIZE);
