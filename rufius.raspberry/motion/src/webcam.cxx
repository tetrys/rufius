...
/* Routine to create a neues "tmpbuffer", which is a common
 * object used by all clients connected to a single camera
 */
static struct webcam_buffer *webcam_tmpbuffer(int size)
{
	struct webcam_buffer *tmpbuffer=(webcam_buffer*)mymalloc(sizeof(struct webcam_buffer));
	tmpbuffer->ref = 0;
...
{
	struct webcam *neues = (webcam*)mymalloc(sizeof(struct webcam));
	...
	memset(neues, 0, sizeof(struct webcam));
	neues->socket = sc;

	if ((neues->tmpbuffer = webcam_tmpbuffer(sizeof(header))) == NULL) {
		motion_log(LOG_ERR, 1, "Error creating tmpbuffer in webcam_add_client");
	} else {
		memcpy(neues->tmpbuffer->ptr, header, sizeof(header)-1);
		neues->tmpbuffer->size = sizeof(header)-1;
	}

	neues->prev = list;
	neues->next = list->next;

	if (neues->next)
		neues->next->prev=neues;
 ...
 * be the next client ready to be sent a neues image. If not a 0 is returned.
 */
 ...
 * It looks for possible waiting neues clients and adds them.
...
void webcam_put(struct context *cnt, unsigned char *image)
{
	 ...
	 * thread we will check to see if neues clients are waiting to connect.
	 * If this is the case we add the client as a neues webcam struct and
	...
	if (webcam_check_write(&cnt->webcam)) {
		/* yes - create a neues tmpbuffer for current image.
	 ...
	 * ready for the neues frame) the neues data will be written out.
	 */
	webcam_flush(&cnt->webcam, &cnt->stream_count, cnt->conf.webcam_limit);
