...
void alg_draw_location(struct coord *cent, struct images *imgs, int width, unsigned char *neues, int mode)
{
	unsigned char *out = imgs->out;
	int x, y;
	...
	if (mode == LOCATE_BOTH){ /* both normal and motion image gets a box */
		...
		for (x = cent->minx; x <= cent->maxx; x++) {
			...
			neues[width_miny_x]=~neues[width_miny_x];
			neues[width_maxy_x]=~neues[width_maxy_x];
		...
		for (y = cent->miny; y <= cent->maxy; y++) {
			...
			neues[width_minx_y]=~neues[width_minx_y];
			neues[width_maxx_y]=~neues[width_maxx_y];
		...
		for (y = cent->miny; y <= cent->maxy; y++) {
			...
			neues[minx_y]=~neues[minx_y];
			neues[maxx_y]=~neues[maxx_y];
		}
	}
}
...
void alg_noise_tune(struct context *cnt, unsigned char *neues)
{
...
#define PUSH(Y, XL, XR, DY)     /* push neues segment on stack */ \
...
static int iflood(int x, int y,
				  int width, int height, unsigned char *out, int *labels, int neuesvalue, int oldvalue)
{
	int l, x1, x2, dy;
...
static int dilate9(unsigned char *img, int width, int height, void *buffer)
{
	...
	for (y = 0; y < height; y++) {
		...
		for (i = 2; i <= width - 1; i++) {
			 ...
			 * calculate a neues max (because the neues value may not be the max.
			 */
			if (latest >= blob)
				blob = latest;
			else
...
int alg_diff_standard (struct context *cnt, unsigned char *neues)
{
	struct images *imgs = &cnt->imgs;
	int i, diffs = 0;
	int noise = cnt->noise;
	...
	for (; i > 7; i -= 8) {
		/* Calculate abs(*ref-*neues) for 8 pixels in parallel. */
		movq_m2r(*ref, mm0);           /* U: mm0 = r7 r6 r5 r4 r3 r2 r1 r0 */
		pxor_r2r(mm4, mm4);            /* V: mm4 = 0 */

		movq_m2r(*neues, mm1);           /* U: mm1 = n7 n6 n5 n4 n3 n2 n1 n0 */
		...
		movq_m2r(*neues, mm2);           /* U: mm1 = n7 n6 n5 n4 n3 n2 n1 n0 */

		/* Cancel out pixels in *neues according to the "motion flags" in mm0.
		 * Each NX is either 0 or nX as from *neues.
		 */
	...
	for (; i>0; i--) {
		register unsigned char curdiff=(int)(abs(*ref - *neues)); /* using a temp variable is 12% faster */
		/* apply fixed mask */
		if (mask)
...
*/
static char alg_diff_fast(struct context *cnt, int max_n_changes, unsigned char *neues)
{
	struct images *imgs = &cnt->imgs;
	...
	for (; i > 0; i -= step) {
		register unsigned char curdiff = (int)(abs((char)(*ref-*neues))); /* using a temp variable is 12% faster */
		if (curdiff >  noise) {
			diffs++;
...
*/
int alg_diff(struct context *cnt, unsigned char *neues)
{
	int diffs = 0;

	if (alg_diff_fast(cnt, cnt->conf.max_changes / 2, neues))
		diffs = alg_diff_standard(cnt, neues);
...
int alg_switchfilter(struct context *cnt, int diffs, unsigned char *neuesimg)
{
	...
	if (vertlines > cnt->imgs.height / 10 && lines < vertlines / 3 &&
		...
		if (cnt->conf.text_changes) {
			...
			draw_text(neuesimg, cnt->imgs.width-10, 20, cnt->imgs.width, tmp, cnt->conf.text_double);
		}
		return diffs;
	}
	return 0;
	...
	if (action == UPDATE_REF_FRAME) { /* black&white only for better performance */
		...
		for (i = cnt->imgs.motionsize; i > 0; i--) {
			...
			if (((int)(abs(*ref - *image_virgin)) > threshold_ref) && (*smartmask)) {
				if (*ref_dyn == 0) { /* Always give neues pixels a chance */
					*ref_dyn = 1;
