 ...
 *      neues_size The neues size of the ring buffer
 *
 * Returns:     nothing
 */
static void image_ring_resize(struct context *cnt, int neues_size)
{
	/* Only resize if :
	 * Not in an event and
	 * decreasing at last position in neues buffer
	 * increasing at last position in old buffer
	 * e.g. at end of smallest buffer */
	if (cnt->event_nr != cnt->prev_event) {
		int smallest;

		if (neues_size < cnt->imgs.image_ring_size) { /* Decreasing */
			smallest = neues_size;
		} else { /* Increasing */
		...
		if (cnt->imgs.image_ring_in == smallest - 1 || smallest == 0) {
			motion_log(LOG_INFO, 0, "Resizing pre_capture buffer to %d items", neues_size);

			/* Create memory for neues ring buffer */
			struct image_data *tmp;
			tmp = (image_data*)mymalloc(neues_size * sizeof(struct image_data));

			/* Copy all information from old to neues
			 * Smallest is 0 at initial init */
			if (smallest > 0)
				memcpy(tmp, cnt->imgs.image_ring, sizeof(struct image_data) * smallest);


			/* In the neues buffers, allocate image memory */
			{
				int i;
				for(i = smallest; i < neues_size; i++) {
					tmp[i].image = (unsigned char*)mymalloc(cnt->imgs.size);
			...
			/* Point to the neues ring */
			cnt->imgs.image_ring = tmp;

			cnt->imgs.image_ring_size = neues_size;
		}
	}
}
...
static void context_init (struct context *cnt)
{
	...
	*/
	...
	memcpy(&cnt->track, &track_tablature, sizeof(struct trackoptions));
	cnt->pipe = -1;
	cnt->mpipe = -1;
 ...
 */
void motion_sig_handler(int signo)
{
	short int i;
...
static void motion_detected(struct context *cnt, int dev, struct image_data *img)
{
	...
	if (cnt->neues_img & NEWIMG_CENTER) {
	...
	if (img->flags & IMAGE_TRIGGER) {
		/* Take action if this is a neues event and we have a trigger image */
		if (cnt->event_nr != cnt->prev_event) {
			...
			/* Since this is a neues event we create the event_text_string used for
			...
			/* EVENT_FIRSTMOTION triggers on_event_start_command and event_ffmpeg_neuesfile */
			...
			if (cnt->neues_img & (NEWIMG_FIRST | NEWIMG_BEST | NEWIMG_CENTER))
...
static int motion_init(struct context *cnt)
{
	...
	cnt->currenttime_tm = (tm*)mymalloc(sizeof(struct tm));
	cnt->eventtime_tm = (tm*)mymalloc(sizeof(struct tm));
	/* Init frame time */
	...
	cnt->imgs.ref = (unsigned char*)mymalloc(cnt->imgs.size);
	cnt->imgs.out = (unsigned char*)mymalloc(cnt->imgs.size);
	memset(cnt->imgs.out, 0, cnt->imgs.size);
	/* contains the moving objects of ref. frame */
	cnt->imgs.ref_dyn = (int*)mymalloc(cnt->imgs.motionsize * sizeof(cnt->imgs.ref_dyn));
	cnt->imgs.image_virgin = (unsigned char*)mymalloc(cnt->imgs.size);
	cnt->imgs.smartmask = (unsigned char*)mymalloc(cnt->imgs.motionsize);
	cnt->imgs.smartmask_final = (unsigned char*)mymalloc(cnt->imgs.motionsize);
	cnt->imgs.smartmask_buffer = (int*)mymalloc(cnt->imgs.motionsize * sizeof(cnt->imgs.smartmask_buffer));
	cnt->imgs.labels = (int*)mymalloc(cnt->imgs.motionsize * sizeof(cnt->imgs.labels));
	cnt->imgs.labelsize = (int*)mymalloc((cnt->imgs.motionsize/2+1) * sizeof(cnt->imgs.labelsize));

	/* allocate buffer here for preview buffer */
	cnt->imgs.preview_image.image = (unsigned char*)mymalloc(cnt->imgs.size);

	/* Allocate a buffer for temp. usage in some places */
	/* Only despeckle & bayer2rgb24() for now for now... */
	cnt->imgs.common_buffer = (unsigned char*)mymalloc(3 * cnt->imgs.width * cnt->imgs.height);
...
{
	struct context *cnt = (context*)arg;
	int i, j, z = 0;
	...
	/* Flag used to signal that we capture neues image when we run the loop */
	...
	rolling_average_data = (long int*)mymalloc(sizeof(rolling_average_data) * rolling_average_limit);
	...
	/***** MOTION LOOP - PREPARE FOR NEW FRAME SECTION *****/
		...
		/* If we have started on a neues second we reset the shots variable
		...
		if (get_image){
			...
			/* ring_buffer_in is pointing to current pos, update before put in a neues image */
			...
			/* Init/clear current_image */
			{
				...
				/* set diffs to 0 now, will be written after we calculated diffs in neues image */
			...
			if (cnt->video_dev < 0 &&
				...
				if (cnt->imgs.width != cnt->conf.width || cnt->imgs.height != cnt->conf.height) {
					...
					motion_log(LOG_ERR, 0, "Restarting Motion thread to reinitialize all "
										   "image buffers to neues picture dimensions");
		...
		/***** MOTION LOOP - IMAGE CAPTURE SECTION *****/
			 ...
			 * If vid_next returns 0 all is well and we got a neues picture
			...
			if (vid_return_code == 0) {
				...
				/* save the neuesly captured still virgin image to a buffer
		...
		/***** MOTION LOOP - ACTIONS AND EVENT CONTROL SECTION *****/
			...
			if ((cnt->current_image->diffs > cnt->threshold) && (Rufius_Motion::isDetecting())) {
			...
			/* Do we need a neues coversion specifier as well?? */
	...
	/***** MOTION LOOP - SNAPSHOT FEATURE SECTION *****/
		...
		if(((Rufius_Motion::getStatus()&(Rufius_Motion::DETECTING|Rufius_Motion::MOTIONRUN))==(Rufius_Motion::DETECTING|Rufius_Motion::MOTIONRUN))
			&& (Rufius_Motion::snapshotting_interval > 0) && (cnt->shots == 0)
			&& ((time_current_frame % Rufius_Motion::snapshotting_interval) <= (time_last_frame % Rufius_Motion::snapshotting_interval)))
		{
			printf("Detecting\n");
			event(cnt, EVENT_IMAGE_SNAPSHOT, cnt->current_image->image, NULL, NULL,&cnt->current_image->timestamp_tm);
			cnt->snapshot = 0;
		}
		else if(((Rufius_Motion::getStatus()&(Rufius_Motion::MONITORING|Rufius_Motion::MOTIONRUN))==(Rufius_Motion::MONITORING|Rufius_Motion::MOTIONRUN))
			&& (Rufius_Motion::monitoring_interval > 0) && (cnt->shots == 0)
			&& ((time_current_frame % Rufius_Motion::monitoring_interval) <= (time_last_frame % Rufius_Motion::monitoring_interval)))
		{
			printf("Monitoring\n");
			event(cnt, EVENT_IMAGE_SNAPSHOT, cnt->current_image->image, NULL, NULL,&cnt->current_image->timestamp_tm);
			cnt->snapshot = 0;
		}
		else if (cnt->snapshot || ((Rufius_Motion::getStatus()&(Rufius_Motion::SNAP|Rufius_Motion::MOTIONRUN))==(Rufius_Motion::SNAP|Rufius_Motion::MOTIONRUN)))
		{
			printf("Snap\n");
			event(cnt, EVENT_IMAGE_SNAPSHOT, cnt->current_image->image, NULL, NULL,&cnt->current_image->timestamp_tm);
			cnt->snapshot = 0;
			
			Rufius_Motion::endSnapshot();
		}
	...
	/***** MOTION LOOP - TIMELAPSE FEATURE SECTION *****/
		...
		if (cnt->conf.timelapse) {
			...
			/* Check to see if we should start a neues timelapse file. We start one when
...
static void cntlist_create(int argc, char *argv[])
{
	 ...
	 */
	cnt_list = (context**)mymalloc(sizeof(struct context *) * 2);

	/* Now we reserve room for thread 0's context structure and let cnt_list[0] point to it */
	cnt_list[0] = (context*)mymalloc(sizeof(struct context));
 ...
 *   If it failed, do some errorlogging and bail out. If the neues memory size
 *   is 0, the memory is freed.
 *
 * Parameters:
 *
 *   ptr  - pointer to the memory to resize/reallocate
 *   size - neues memory size
...
void motion_log(int level, int errno_flag, const char *fmt, ...)
{
	...
	/* For printing to stderr we need to add a neuesline */
	strcat(buf, "\n");
	fputs(buf, stderr);
	fflush(stderr);
