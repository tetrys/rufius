...
int mjpegtoyuv420p(unsigned char *map, unsigned char *cap_map, int width, int height, unsigned int size)
{
	...
	yuv[0] = (uint8_t*)malloc(width * height * sizeof(yuv[0][0]));
	yuv[1] = (uint8_t*)malloc(width * height / 4 * sizeof(yuv[1][0]));
	yuv[2] = (uint8_t*)malloc(width * height / 4 * sizeof(yuv[2][0]));
 ...
 *                each thread adds a neues video device during startup calling vid_start
 *
 * Returns
 *     device number
...
static int vid_v4lx_start(struct context *cnt)
{
	...
	dev = (video_dev*)mymalloc(sizeof(struct video_dev));
 ...
 * Parameters:
 *     cnt        Pointer to the context for this thread
 *     map        Pointer to the buffer in which the function puts the neues image
 *
 * Global variable
 *     viddevs    The viddevs struct is "global" within the context of video.c
 *                and used in functions vid_*.
...
#ifndef WITHOUT_V4L
	/* We start a neues block so we can make declarations without breaking
	 * gcc 2.95 or older
	 */
	{
		struct video_dev *dev;
