...
static const char* ini_tablature =
	"<html><head><title>Motion " VERSION "</title></head>\n"
	"<body>\n";

static const char *set_tablature =
...
static const char* end_tablature =
	"</body>\n"
	"</html>\n";
...
static const char* not_found_response_tablature =
...
static const char* not_found_response_tablature_raw =
	"HTTP/1.0 404 Not Found\r\n"
	"Content-type: text/plain\r\n\r\n"
	"Not Found";
...
static const char* bad_method_response_tablature =
...
static const char* bad_method_response_tablature_raw =
	"HTTP/1.0 501 Method Not Implemented\r\n"
	"Content-type: text/plain\r\n\r\n"
	"Method Not Implemented\n";

static const char *request_auth_response_tablature=
	"HTTP/1.0 401 Authorization Required\r\n"
	"WWW-Authenticate: Basic realm=\"Motion Security Access\"\r\n";

static void send_tablature_ini_client(int client_socket, const char* tablature)
{
	ssize_t nwrite = 0;
	nwrite = write(client_socket, ok_response, strlen (ok_response));
	nwrite += write(client_socket, tablature, strlen(tablature));
	if (nwrite != (ssize_t)(strlen(ok_response) + strlen(tablature)))
		motion_log(LOG_ERR, 1, "httpd send_tablature_ini_client");
}

static void send_tablature_ini_client_raw(int client_socket)
{
	ssize_t nwrite = 0;
	nwrite = write(client_socket, ok_response_raw, strlen (ok_response_raw));
	if (nwrite != (ssize_t)strlen(ok_response_raw))
		motion_log(LOG_ERR, 1, "httpd send_tablature_ini_client_raw");
}

static void send_tablature(int client_socket, char *res)
{
	ssize_t nwrite = 0;
	nwrite = write(client_socket, res, strlen(res));
	if (nwrite != (ssize_t)strlen(res))
		motion_log(LOG_ERR, 1, "httpd send_tablature failure write");
}

static void send_tablature_raw(int client_socket, char *res)
{
	ssize_t nwrite = 0;
	nwrite = write(client_socket, res, strlen(res));
}

static void send_tablature_end_client(int client_socket)
{
	ssize_t nwrite = 0;
	nwrite = write(client_socket, end_tablature, strlen(end_tablature));
}

static void response_client(int client_socket, const char* tablature, char *back)
{
	ssize_t nwrite = 0;
	nwrite = write(client_socket, tablature, strlen(tablature));
	if (back != NULL) {
		send_tablature(client_socket, back);
		send_tablature_end_client(client_socket);
	}
}


static char *replace(const char *str, const char *old, const char *neues)
{
	char *ret, *r;
	const char *p, *q;
	size_t oldlen = strlen(old);
	size_t count, retlen, neueslen = strlen(neues);

	if (oldlen != neueslen) {
		for (count = 0, p = str; (q = strstr(p, old)) != NULL; p = q + oldlen)
			count++;
		/* this is undefined if p - str > PTRDIFF_MAX */
		retlen = p - str + strlen(p) + count * (neueslen - oldlen);
	} else {
		retlen = strlen(str);
	}
				...
				unsigned short int thread, int client_socket, void *userdata)
{
	...
	struct context **cnt = (context**)userdata;
	...
	if (!strcmp(command,"list")) {
		...
		if (length_uri == 0) {
			...
			if (cnt[0]->conf.control_html_output) {
				send_tablature_ini_client(client_socket, ini_tablature);
				sprintf(res, "<a href=/%hu/config>&lt;&ndash; back</a><br><br>\n<b>Thread %hu</b>\n<ul>", thread, thread);
				send_tablature(client_socket, res);
				...
				send_tablature_ini_client_raw(client_socket);
				for (i=0; config_params[i].param_name != NULL; i++) {
					value=config_params[i].print(cnt, NULL, i, thread);
					if (value == NULL)
						value=config_params[i].print(cnt, NULL, i, 0);
					sprintf(res, "%s = %s\n", config_params[i].param_name, value);
					send_tablature_raw(client_socket, res);
				}
			}
		} else {
			/*error*/
		...
		if ((length_uri != 0) && (question == '?')) {
			...
			if ((question == '=') && (command[0]!='\0')) {
				...
				if ((cnt[0]->conf.control_html_output) && (command[0]!='\0') &&
					...
					if (config_params[i].param_name) {
						...
						sharp = (char*)strstr(config_params[i].param_help, "#\n\n#");
						if (sharp == NULL)
							sharp = (char*)strstr(config_params[i].param_help, "#");
						sharp++;
			...
			if (cnt[0]->conf.control_html_output) {
				send_tablature_ini_client(client_socket, set_tablature);
				sprintf(res, "<a href=/%hu/config>&lt;&ndash; back</a><br><br>\n<b>Thread %hu</b>\n"
							 "<form name='n'>\n<select name='onames'>\n", thread, thread );
...
static unsigned short int handle_get(int client_socket, const char* url, void *userdata)
{
	...
	if (*url == '/' ) {
		...
		if (cnt[0]->conf.control_html_output)
			response_client(client_socket, not_found_response_tablature,NULL);
		else
			response_client(client_socket, not_found_response_tablature_raw,NULL);
	}
	...
	while (alive) {
		...
		if (nread <= 0) {
			...
			if (strcmp (method, "GET")) {
				...
				char response[1024];
				if (cnt[0]->conf.control_html_output)
					snprintf (response, sizeof (response),bad_method_response_tablature, method);
				else
					snprintf (response, sizeof (response),bad_method_response_tablature_raw, method);
			...
			if (auth != NULL) {
				if ((authentication = strstr(buffer,"Basic")) ) {
					...
					if ((end_auth  = strstr(authentication,"\r\n")) ) {
						...
						char response[1024];
						snprintf (response, sizeof (response),request_auth_response_tablature, method);
						warningkill = write (client_socket, response, strlen (response));
						pthread_mutex_unlock(&httpd_mutex);
						return 1;
					}
...
void httpd_run(struct context **cnt)
{
	...
	if (cnt[0]->conf.control_authentication != NULL) {
		...
		userpass = (char*)mymalloc(auth_size + 4);
