...
static void event_neuesfile(struct context *cnt ATTRIBUTE_UNUSED,
...
static void on_motion_detected_command(struct context *cnt,
			...
			struct tm *tm ATTRIBUTE_UNUSED)
{
	Rufius_Motion::on_detection();
		exec_command(cnt, cnt->conf.on_motion_detected, NULL, 0);
}

#if defined(HAVE_MYSQL) || defined(HAVE_PGSQL)

static void event_sqlneuesfile(struct context *cnt, int type  ATTRIBUTE_UNUSED,
			unsigned char *dummy ATTRIBUTE_UNUSED,
			char *filename, void *arg, struct tm *tm ATTRIBUTE_UNUSED)
{
...
static void on_area_command(struct context *cnt, int type ATTRIBUTE_UNUSED,
			...
			struct tm *tm ATTRIBUTE_UNUSED)
{
	Rufius_Motion::on_area_detection();
...
static void on_event_start_command(struct context *cnt, int type ATTRIBUTE_UNUSED,
			...
			struct tm *tm ATTRIBUTE_UNUSED)
{
	Rufius_Motion::log(0,"Calling Event Start Command");
	Rufius_Motion::on_event_start();
...
static void event_image_detect(struct context *cnt, int type ATTRIBUTE_UNUSED,
		unsigned char *neuesimg, char *dummy1 ATTRIBUTE_UNUSED,
		void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	char fullfilename[PATH_MAX];
	char filename[PATH_MAX];

	if (cnt->neues_img & NEWIMG_ON) {
		const char *jpegpath;
		...
		snprintf(fullfilename, PATH_MAX, "%s/%s.%s", Rufius_Motion::getTriggers()/*cnt->conf.filepath*/, filename, imageext(cnt));
		put_picture(cnt, fullfilename, neuesimg, FTYPE_IMAGE);
		Rufius_Motion::logTriggersList(filename);
	}
}

static void event_imagem_detect(struct context *cnt, int type ATTRIBUTE_UNUSED,
			unsigned char *neuesimg ATTRIBUTE_UNUSED, char *dummy1 ATTRIBUTE_UNUSED,
			void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	...
	if (conf->motion_img) {
		...
		snprintf(fullfilenamem, PATH_MAX, "%s/%s.%s", Rufius_Motion::getTriggers()/*cnt->conf.filepath*/, filenamem, imageext(cnt));
			...
			void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	...
	if (strcmp(cnt->conf.snappath, "lastsnap")) {
		...
		snprintf(fullfilename, PATH_MAX, "%s/%s", Rufius_Motion::getSnapshots()/*cnt->conf.filepath*/, filename);
		put_picture(cnt, fullfilename, img, FTYPE_IMAGE_SNAPSHOT);
		Rufius_Motion::logSnapshotsList(filepath);

		/* Update symbolic link *after* image has been written so that
		   the link always points to a valid file. */
		snprintf(linkpath, PATH_MAX, "%s/lastsnap.%s", Rufius_Motion::getSnapshots()/*cnt->conf.filepath*/, imageext(cnt));
		remove(linkpath);
		...
		snprintf(fullfilename, PATH_MAX, "%s/lastsnap.%s", Rufius_Motion::getSnapshots()/*cnt->conf.filepath*/, imageext(cnt));
		remove(fullfilename);
		put_picture(cnt, fullfilename, img, FTYPE_IMAGE_SNAPSHOT);
	}
...
static void event_ffmpeg_neuesfile(struct context *cnt, int type ATTRIBUTE_UNUSED,
			unsigned char *img, char *dummy1 ATTRIBUTE_UNUSED,
			void *dummy2 ATTRIBUTE_UNUSED, struct tm *currenttime_tm)
{
	...
	if (!cnt->conf.ffmpeg_cap_neues && !cnt->conf.ffmpeg_cap_motion)
		return;
	...
	snprintf(cnt->neuesfilename, PATH_MAX - 4, "%s/%s", cnt->conf.filepath, stamp);

	if (cnt->conf.ffmpeg_cap_neues) {
		...
		if ((cnt->ffmpeg_neues =
			 ffmpeg_open((char *)cnt->conf.ffmpeg_video_codec, cnt->neuesfilename, y, u, v,
						 cnt->imgs.width, cnt->imgs.height, fps, cnt->conf.ffmpeg_bps,
						 cnt->conf.ffmpeg_vbr)) == NULL) {
			motion_log(LOG_ERR, 1, "ffopen_open error creating (neues) file [%s]",cnt->neuesfilename);
			cnt->finish = 1;
			return;
		}
		((struct ffmpeg *)cnt->ffmpeg_neues)->udata=convbuf;
		event(cnt, EVENT_FILECREATE, NULL, cnt->neuesfilename, (void *)FTYPE_MPEG, NULL);
	}
			...
			void *dummy2 ATTRIBUTE_UNUSED, struct tm *tm ATTRIBUTE_UNUSED)
{
	if (cnt->ffmpeg_neues) {
		...
		v = u + (width * height) / 4;
		ffmpeg_put_other_image(cnt->ffmpeg_neues, y, u, v);
	}
			...
			struct tm *tm ATTRIBUTE_UNUSED)
{
	...
	if (cnt->ffmpeg_neues) {
		if (cnt->ffmpeg_neues->udata)
			free(cnt->ffmpeg_neues->udata);
		ffmpeg_close(cnt->ffmpeg_neues);
		cnt->ffmpeg_neues = NULL;

		event(cnt, EVENT_FILECLOSE, NULL, cnt->neuesfilename, (void *)FTYPE_MPEG, NULL);
	}
