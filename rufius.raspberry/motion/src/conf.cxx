...
#define stripneuesline(x) {if ((x)[strlen(x)-1]=='\n') (x)[strlen(x) - 1] = 0;}


struct config conf_tablature = {
	0,
	DEF_WIDTH,
	DEF_HEIGHT,
	DEF_QUALITY,
	...
	NULL,
	NULL,
	"1.0",    /* Choices: 1.0, 1.1, or keep_alive */
	NULL,
	0,
	NULL,
	"localhost",
...
config_param config_params[] = {
	...
	"# When value is exceeded a neues mpeg file is created. (Default: 0 = infinite)",
	0,
...
struct context **conf_cmdparse(struct context **cnt, const char *cmd, const char *arg1)
{
	...
	while (config_params[i].param_name != NULL) {
		if (!strncasecmp(cmd, config_params[i].param_name , 255 + 50)) { // Why +50?
			 ...
			 *  arg1 - a pointer to the neues option value (represented as string)
 ...
 * - The default start values for cnt stored in the struct conf_tablature
 ...
 *   the string given in conf_tablature.
...
struct context **conf_load(struct context **cnt)
{
	...
	/* Copy the tablature config structure with all the default config values
	 * into cnt[0]->conf
	 */
	memcpy(&cnt[0]->conf, &conf_tablature, sizeof(struct config));

	/* For each member of cnt[0] which is a pointer to a string
	 * if the member points to a string in conf_tablature and is not NULL
	 * 1. Reserve (malloc) memory for the string
	 * 2. Copy the conf_tablature given string to the reserved memory
...
void malloc_strings(struct context * cnt)
{
	...
	while (config_params[i].param_name != NULL) {
		if (config_params[i].copy == copy_string) { /* if member is a string */
			 ...
			 * the string to neues space, and point to the neues
...
/* copy_bool assigns a config option to a neues boolean value.
...
/* copy_int assigns a config option to a neues integer value.
...
/* copy_short assigns a config option to a neues short value.
...
/* copy_string assigns a neues string value to a config option.
...
struct context **copy_string(struct context **cnt, const char *str, int val_ptr)
{
	...
	while (cnt[++i]) {
		...
		/* mystrcpy assigns the neues string value
		 * including free'ing and reserving neues memory for it.
		 */
 ...
 * old string is free'd and neues memory is malloc'ed and filled with the
 * neues string is copied into the the memory and with the char pointer
 * pointing to the neues string.
 *
 * from - pointer to the neues string we want to copy
 * to   - the pointer to the current string (or pointing to NULL)
 *        If not NULL the memory it points to is free'd.
 * function returns pointer to the neues string which is in malloc'ed memory
								 ...
								 int parm ATTRIBUTE_UNUSED, unsigned short int threadnr)
{
	...
	retval[0] = 0;
	while (cnt[++i]) {
		retval = (char*)myrealloc(retval, strlen(retval) + strlen(cnt[i]->conf_filename) + 10, "print_thread");
									   ...
									   int val ATTRIBUTE_UNUSED)
{
	 ...
	 */
	cnt = (context**)myrealloc(cnt, sizeof(struct context *) * (i + 2), "config_thread");

	/* Now malloc space for an additional context structure for thread nr. i */
	cnt[i] = (context*)mymalloc(sizeof(struct context));
	 ...
	 * change the string pointers to point to the neues strings.
