...
#        warning **************************************************
#        warning Your version of FFmpeg is neueser than version 0.4.8
...
/* This function opens an mpeg file using the neues libavformat method. Both mpeg1
                           ...
                           int width, int height, int rate, int bps, int vbr)
{
    ...
    /* Create a neues video stream and initialize the codecs */
    ffmpeg->video_st = NULL;

    if (ffmpeg->oc->oformat->video_codec != CODEC_ID_NONE) {
        ffmpeg->video_st = av_neues_stream(ffmpeg->oc, 0);
        if (!ffmpeg->video_st) {
            motion_log(LOG_ERR, 1, "av_neues_stream - could not alloc stream");
            ffmpeg_cleanups(ffmpeg);
            return NULL;
        }
    } else {
